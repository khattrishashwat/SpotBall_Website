...
function Checkout() {
  ...
  useEffect(() => {
    if (calculatedCarts.length > 0) {
      const updatedCart = calculatedCarts[0]; // Assuming a single cart for simplicity
      const discounts = calculateDiscounts(updatedCart);
      console.log("Updated Discounts:", discounts);
      // Optionally, update state with discounts if you want to render them.
    }
  }, [calculatedCarts]);
  ...
  //   if (appliedPromoCode?.name === promoCode) {
  //     // Swal.fire({
  //     //   icon: "info",
  ...
  //       icon: "error",
  //       title: "Cart is Empty",
  //       text: "Add items before applying a promo code.",
  //     });
  //     return;
  //   }

  //   // Apply discount logic as percentage
  //   const updatedCarts = calculatedCarts.map((cart) => {
  //     const totalPayment = parseFloat(cart.totalPayment);
  //     // Calculate discount percentage value (promo.amount is treated as a percentage)
  //     const discountValue = (promo.amount / 100) * totalPayment;
  //     // Ensure discount does not exceed totalPayment
  //     const finalTotal = Math.max(totalPayment - discountValue, 0);
  ...
  //       promoCode: {
  //         name: promo.name,
  //         amount: promo.amount, // percentage
  //       },
  //       totalPayment: finalTotal.toFixed(2),
  //     };
  //   });
  ...
  //   // Prepare API call using updated cart data
  ...
  //   // If a promoCode is applied, treat its amount as a percentage discount on the totalTicketPrice
  //   const promoDiscountName = cart.promoCode ? cart.promoCode.name : "";
  //   const promoDiscountValue = cart.promoCode
  //     ? (cart.promoCode.amount / 100) * cart.totalTicketPrice
  //     : 0;

  //   // For discounts, we assume that the cart object has a discount object injected.
  ...
  //   // Calculate the discounted ticket price subtracting both the flat discount and the percentage promo discount
  //   const discountedTotalTicketPrice = Math.max(
  //     cart.totalTicketPrice - discountPerCart - promoDiscountValue,
  //     0
  //   ).toFixed(2);
  ...
  //   const result = {
  //     promoDiscountName,
  //     // Return the calculated promo discount value (as a flat amount)
  //     promoDiscountAmount: promoDiscountValue.toFixed(2),
  //     PerDiscount,
  //     discountPerCart,
  //     discountedTotalTicketPrice,
  //     discountedGstAmount,
  ...
  //   // Log each returned property to the console
  //   console.log("Promo Discount Name:", result.promoDiscountName);
  //   console.log("Promo Discount Amount:", result.promoDiscountAmount);
  //   console.log("Percentage Discount (PerDiscount):", result.PerDiscount);
  //   console.log("Flat Discount Per Cart:", result.discountPerCart);
  //   console.log(
  //     "Discounted Total Ticket Price:",
  //     result.discountedTotalTicketPrice
  //   );
  //   console.log("Discounted GST Amount:", result.discountedGstAmount);
  //   console.log("Discounted Subtotal:", result.discountedSubtotal);
  //   console.log("Discounted Platform Fee:", result.discountedPlatformFee);
  //   console.log(
  //     "Discounted GST on Platform Fee:",
  //     result.discountedGstOnPlatformFee
  //   );
  //   console.log(
  //     "Discounted Total Razorpay Fee:",
  //     result.discountedTotalRazorpayFee
  //   );
  //   console.log("Discounted Grand Total:", result.discountedGrandTotal);

  //   return result;
  // };

  const calculateDiscounts = (cart) => {
    const { totalTicketPrice, promoCode, discount, contest_id } = cart;

    // Promo Discount
    const promoDiscountAmount = promoCode
      ? (totalTicketPrice * promoCode.amount) / 100
      : 0;
    const discountedTotalTicketPrice = totalTicketPrice - promoDiscountAmount;

    // Additional Discount
    const PerDiscount = discount ? discount.percentage : 0;
    const discountPerCart = discount
      ? (discountedTotalTicketPrice * PerDiscount) / 100
      : 0;
    const finalTicketPrice = discountedTotalTicketPrice - discountPerCart;

    // GST Calculation
    const gstRate = contest_id.gstRate || 0;
    const discountedGstAmount = (finalTicketPrice * gstRate) / 100;

    // Subtotal
    const discountedSubtotal = finalTicketPrice + discountedGstAmount;

    // Platform Fee
    const platformFeeRate = contest_id.platformFeeRate || 0;
    const discountedPlatformFee = (discountedSubtotal * platformFeeRate) / 100;

    // GST on Platform Fee
    const gstOnPlatFormFeeRate = contest_id.gstOnPlatformFeeRate || 0;
    const discountedGstOnplatformFee =
      (discountedPlatformFee * gstOnPlatFormFeeRate) / 100;

    // Total Platform Fee
    const discountedTotalRazFee =
      discountedPlatformFee + discountedGstOnplatformFee;

    // Grand Total
    const discountGrandTotal = discountedSubtotal + discountedTotalRazFee;

    return {
      promoDiscountAmount,
      PerDiscount,
      discountPerCart,
      discountedTotalTicketPrice: finalTicketPrice,
      discountedGstAmount,
      discountedSubtotal,
      discountedPlatformFee: gstOnPlatFormFeeRate.toFixed(2),
      discountedGstOnPlatformFee: discountedGstOnplatformFee.toFixed(2),
      discountedTotalRazorpayFee: discountedTotalRazFee.toFixed(2),
      discountedGrandTotal: discountGrandTotal.toFixed(2),
    };
  };

  const totalBeforeDiscount = calculatedCarts.reduce((total, cart) => {
    // Convert the discountedGrandTotal to a number just in case it's a string
    const discountedTotal = Number(
      calculateDiscounts(cart).discountedGrandTotal
    );
  ...
  console.log("totalBeforeDiscount", totalBeforeDiscount); // Should output 83.81 without the extra leading zero

  useEffect(() => {
    if (calculatedCarts.length > 0) {
      console.log("Updated Discounts:", calculateDiscounts(calculatedCarts[0]));
    }
  }, [calculatedCarts]);
  ...
  const handleApplyPromoCode = async () => {
    ...
    if (appliedPromoCode?.name === promoCode) return;
    ...
    const updatedCarts = calculatedCarts.map((cart) => {
      const discountValue =
        (promo.amount / 100) * parseFloat(cart.totalPayment);
      const finalTotal = Math.max(
        parseFloat(cart.totalPayment) - discountValue,
        0
      );
      ...
      return {
        ...cart,
        discount:null,
        promoCode: { name: promo.name, amount: promo.amount },
        totalPayment: finalTotal.toFixed(2),
      };
    });
    ...
    const payload = {
      ...
      tickets_count: updatedCart?.tickets_count,
      user_coordinates: updatedCart?.user_coordinates,
      promocodeApplied: { name: promo.name, amount: promo.amount },
    };
  ...
  const Money = async () => {
    try {
      const token = localStorage.getItem("Web-token");
      const order_amount = totalBeforeDiscount;
